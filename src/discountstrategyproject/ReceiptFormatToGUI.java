...
public class ReceiptFormatToGUI implements ReceiptFormatter{
    ...
    public final void displayReceipt(List<LineItem> items) {
        ...
        for(LineItem item: items){
            ...
            JOptionPane.showMessageDialog(null, item.getProdName() + "   " + Math.floor(item.getUnitPrice() * 100) / 100 + "   " + item.getQty() + "   " + Math.floor(item.getSubtotal() * 100) / 100 + "   " + Math.floor(item.getDiscountAmt() * 100) / 100);
        }
